...
| 2 | image           | アバター / 任意アップロード / アニメ画像                          |
| 3 | shape           | 矩形・円（塗り/枠線/角丸）                                        |
| 4 | qr              | プロフィールURL or 自由入力                                       |
| 5 | animeImage      | お気に入りアニメから選択したポスター（image の source="anime" で表現）|
| 6 | animeCollage    | 視聴履歴コラージュ（sortBy/limit/gap、実データは動的取得）        |
| 7 | animeCountBadge | 視聴数バッジ（prefix/suffix + metric、実カウントは動的取得）      |

（textとimageは source フィールドで内部種別を分けるため、実質的な type は 5 つ）
