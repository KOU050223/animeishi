...
describe("getAuthError", () => {
  it("サインイン系エラーコードに対してメッセージを返す", () => {
    expect(getAuthError("form_identifier_not_found")).toBe(
      "このメールアドレスのアカウントは存在しません",
    );
    expect(getAuthError("form_password_incorrect")).toBe(
      "パスワードが間違っています",
    );
  ...
  it("サインアップ系エラーコードに対してメッセージを返す", () => {
    expect(getAuthError("form_email_conflict")).toBe(
      "このメールアドレスは既に使用されています",
    );
    expect(getAuthError("form_invalid_email_address")).toContain(
      "メールアドレス",
    );
    expect(getAuthError("form_password_not_strong_enough")).toContain(
      "弱すぎます",
    );
    expect(getAuthError("form_invalid_password_length_too_short")).toContain(
      "8文字以上",
    );
  });

  it("セッション系エラーコードに対してメッセージを返す", () => {
    expect(getAuthError("session_reverification_required")).toContain(
      "再ログイン",
    );
